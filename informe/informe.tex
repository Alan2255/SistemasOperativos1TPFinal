...
\begin{itemize}
    \item \textbf{Dallas Cañari Benites - Ingeniero de comunicaciones (C)}: responsable de la implementación del servidor en C utilizando \texttt{epoll}, manejo de sockets TCP/UDP, comunicación no bloqueante y gestión de buffers.

    \item \textbf{Alan Hergenreder - Gestor de recursos}: encargado de la administración de recursos del sistema, estructuras de datos, colas FIFO, asignación y liberación de recursos, y control de jobs activos.

    \item \textbf{Valentino Perticarati - Planificador y lógica anti-deadlock (Erlang)}: desarrollo del planificador en Erlang, generación de jobs, comunicación con el servidor en C y diseño de estrategias de prevención/detección de deadlocks.

    \item \textbf{Benjamín Alomar - Integración, testing y documentación}: responsable de la integración entre módulos, scripts de prueba, validación de interoperabilidad y documentación del sistema.
\end{itemize}
...
La tabla \texttt{pendientes} almacena, para cada job activo, su identificador,
...
La tabla \texttt{recursos\_nodos} mantiene la disponibilidad de CPU, MEMORIA
...
Al tratarse de tablas ETS públicas, todos los procesos del scheduler pueden
...
Por ejemplo, si un job necesita cuatro CPUs y ningún nodo posee las cuatro disponibles, el scheduler puede construir una petición que combine recursos provenientes de distintos nodos.

Esta estrategia busca aprovechar mejor los recursos globales del sistema y aumentar la probabilidad de ejecución de los trabajos.

Durante este proceso, el scheduler realiza reservas locales descontando inmediatamente los recursos seleccionados de la tabla ETS mediante operaciones atómicas, antes de enviar el \texttt{JOB\_REQUEST}. De esta manera, otros jobs consideran esos recursos como ocupados y no pueden asignarlos simultáneamente. Si el trabajo finaliza correctamente (\texttt{JOB\_GRANTED}), es rechazado (\texttt{JOB\_DENIED}) o expira por timeout, dichas reservas se revierten automáticamente, restaurando los recursos en la tabla ETS y manteniendo consistente el estado local del scheduler.

\section{Problemas encontrados y soluciones}

Durante el desarrollo del agente en C surgieron varios problemas de concurrencia que se fueron detectando y corrigiendo a medida que se probaba el sistema. A continuación se describen los más relevantes.
