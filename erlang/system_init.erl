...
% Recibe: JobTimeout(int en milisegundos), Socket, Pid_caller(int, quien inicializo el sistema y espera la confirmacion)
...
supervisor_scheduler_jobs(JobTimeout, Socket, Pid_caller, TimeJob) ->
    ...
    % Avisamos al inicializador que ya esta todo montado
    Pid_caller ! scheduler_listo,
    
    % Saltamos al bucle de escucha perpetuo pasandole el PID del scheduler actual
...
% Distingue terminacion normal de un crash (revive el proceso con
% un nuevo Pid), e ignora EXIT de cualquier otro proceso que no sea el scheduler actual.

% Recibe: JobTimeout(int), Socket, Pid_scheduler_job(Pid del scheduler que se esta supervisando)
% No retorna nada relevante: corre indefinidamente (o hasta terminacion normal del scheduler).
bucle_supervisor(JobTimeout, Socket, Pid_scheduler_job, TimeJob) ->
    receive 
        {'EXIT', Pid_scheduler_job, normal} ->
            % El scheduler termino de procesar todo de forma limpia. 
        ...
        {'EXIT', _OtroPid, _Reason} ->
            % Se cayo otra cosa (por ejemplo el padre o wait_jobs)
            % Seguimos escuchando con el mismo PID de scheduler de antes
            bucle_supervisor(JobTimeout, Socket, Pid_scheduler_job, TimeJob)
    end.

% Pide el mapa de nodos al agente C de forma SiNCRONA y bloqueante, leyendo el socket directamente (sin pasar por tcp_deliver,
% que todavia no existe en este  punto del arranque). Se usa una unica vez, al inicio del sistema.

% Recibe: Socket
% Retorna: MapNodos (map de Host => [CantCPU, CantMEM, CantGPU]).
% Si falla la conexion, termina el proceso con exit({error_pidiendo_mapa_inicial, Reason}).
...
% Convierte el mapa inicial {Host => [CPU,MEM,GPU]} en entradas atomicas por recurso,
% mas una lista con el orden de los nodos, y las inserta en la tabla ETS 'recursos_nodos'.
% Esto permite despues usar ets:update_counter de forma atomica al repartir jobs.
% EJ:            CLAVE          VALOR
%           { {"Nodo1", 1}   ,   4}
%           { {"Nodo1", 2}   ,   9}
...
% levanta el supervisor del scheduler (esperando su confirmacion antes de seguir), y finalmente arranca
% tcp_deliver para empezar a escuchar respuestas del agente en tiempo de ejecucion.
% Recibe: Puerto(int)
% Retorna: Socket (para que quien llamo pueda usarlo, ej: manual_loop en modo manual)
inicializar_sistema(Puerto, TimeJob, JobTimeoutInit) -> 
    ...
    % Pedimos el mapa de nodos DIRECTO por el socket, en modo sincrono,
    % sin pasar por tcp_deliver (que todavia no existe).
