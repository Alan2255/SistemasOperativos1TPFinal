...
typedef struct {
    ...
    int front;                           // indice al primer elemento (para desencolar)
    int rear;                            // indice al ultimo elemento (para encolar)
    int count;                           // Cuantos jobs hay esperando actualmente
} JobQueue;
...
typedef struct {
    ...
    int total_capacity;
    int available;
    JobQueue job_pendings; // La cola de jobs en espera para ESTE recurso especifico
